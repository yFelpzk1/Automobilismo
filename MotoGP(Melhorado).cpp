...
void Ler_Pontos(MotoGP *pPiloto) {
    ...
    cout << endl << "Digite quantas corridas (provas de Domingo) tem restantes: ";
    cin >> pPiloto->Corridas;
    cout << "Digite quantas sprints (provas de Sábado) tem restantes: ";
    cin >> pPiloto->Sprints;
    cin.ignore(); 
    ...
    cout << "Digite a posição ATUAL do seu piloto (ex: 6): ";
    ...
    cin.ignore(); 
    cout << endl << "--- Campeonato de Equipes (Construtores) ---" << endl;
    ...
    cout << "Digite a posição ATUAL da sua equipe (ex: 4): ";
...
void Calcular_Resultado(MotoGP *pPiloto) {
    // 1. Calcula o máximo de pontos
    // Piloto: Max 25 (Corrida) + 12 (Sprint)
    // Equipe: Max 45 (Corrida 1-2) + 21 (Sprint 1-2)
    
    float maxPontosPiloto = pPiloto->pontosPiloto + (pPiloto->Corridas * 25) + (pPiloto->Sprints * 12);
    float maxPontosConstrutor = pPiloto->pontosConstrutor + (pPiloto->Corridas * 45) + (pPiloto->Sprints * 21);

    cout << endl << "-----------------------------------------------" << endl;
    cout << "--- Potencial Máximo (Simulação MotoGP) ---" << endl;
